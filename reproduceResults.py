...
corr_frame[:,:,:]=corrConv.corr(now_frame,next_frame,corr_support) #todo - next frame is zeros?
...
while(cap.isOpened() and ret):
    ...
    #randframe=np.uint8(np.random.rand(now_frame.shape[0],now_frame.shape[1],3)*rand_mult)
    corr_frame[:,:,:]=corrConv.corr(now_frame*(1.-1./rand_mult)+randframe,next_frame*(1.-1./rand_mult)+randframe,corr_support)
    print time.time()-tic
